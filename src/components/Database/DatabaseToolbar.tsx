...
export default function DatabaseToolbar({ boardId }: DatabaseToolbarProps) {
  ...
  return (
    <div className="border-b border-gray-200 dark:border-[#30363D] bg-white dark:bg-[#1E252B] px-4 py-3 flex items-center gap-3">
      {/* Add Row Button */}
      <button
          ...
          <div className="absolute top-full left-0 mt-1 w-64 bg-white dark:bg-[#1E252B] border border-gray-200 dark:border-[#30363D] rounded-lg shadow-lg z-50 py-1 max-h-96 overflow-y-auto">
            ...
            {getMVPPropertyTypes().map((typeInfo) => (
              <button
                ...
                className="w-full px-3 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-[#252B32] flex items-center gap-3"
              >
                <span className="text-lg">{typeInfo.icon}</span>
                <div className="flex-1">
                  <div className="font-medium text-gray-900 dark:text-[#E0E6ED]">
                    {typeInfo.label}
                  </div>
                  <div className="text-xs text-gray-500 dark:text-[#B1B9C4]">
      ...
      {/* Divider */}
      <div className="h-6 w-px bg-gray-200 dark:bg-[#252B32]" />

      {/* Filter Button */}
      <button
        onClick={() => setShowFilterModal(true)}
        className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-gray-700 dark:text-[#B1B9C4] bg-gray-100 dark:bg-[#252B32] hover:bg-gray-200 dark:hover:bg-[#2C333A] rounded-md transition-colors relative"
        title="Filters"
      >
