/**
 * EmailCell Component
 * Email editor with mailto link
 */

import { useState, useRef, useEffect } from 'react';
import { Mail } from 'lucide-react';

interface EmailCellProps {
  value: string;
  onChange: (value: string) => void;
}

export default function EmailCell({ value, onChange }: EmailCellProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [editValue, setEditValue] = useState(value || '');
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    setEditValue(value || '');
  }, [value]);

  useEffect(() => {
    if (isEditing && inputRef.current) {
      inputRef.current.focus();
      inputRef.current.select();
    }
  }, [isEditing]);

  const handleSave = () => {
    if (editValue !== value) {
      onChange(editValue);
    }
    setIsEditing(false);
  };

  const handleCancel = () => {
    setEditValue(value || '');
    setIsEditing(false);
  };

  if (isEditing) {
    return (
      <input
        ref={inputRef}
        type="email"
        value={editValue}
        onChange={(e) => setEditValue(e.target.value)}
        onBlur={handleSave}
        onKeyDown={(e) => {
          if (e.key === 'Enter') {
            e.preventDefault();
            handleSave();
          }
          if (e.key === 'Escape') {
            e.preventDefault();
            handleCancel();
          }
        }}
        placeholder="email@example.com"
        className="w-full px-2 py-1 text-sm bg-white dark:bg-[#252B32] border border-primary-500 dark:border-primary-400 rounded focus:outline-none focus:ring-1 focus:ring-primary-500 dark:focus:ring-primary-400 text-gray-900 dark:text-[#E0E6ED]"
      />
    );
  }

  if (value) {
    return (
      <div className="flex items-center gap-2 px-2 py-1 text-sm group">
        <Mail className="w-3.5 h-3.5 text-gray-400 dark:text-[#6B7280] flex-shrink-0" />
        <a
          href={`mailto:${value}`}
          className="flex-1 text-primary-600 dark:text-primary-400 hover:underline truncate"
        >
          {value}
        </a>
        <button
          onClick={() => setIsEditing(true)}
          className="text-xs text-gray-400 hover:text-gray-600 dark:text-[#6B7280] dark:hover:text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity"
        >
          Modifier
        </button>
      </div>
    );
  }

  return (
    <div
      onClick={() => setIsEditing(true)}
      className="w-full px-2 py-1 text-sm cursor-text hover:bg-gray-50 dark:hover:bg-[#252B32]/50 rounded min-h-[28px] text-gray-400 dark:text-[#6B7280] italic"
    >
      Ajouter un email...
    </div>
  );
}
